...
# %%
# For each run
denoised_timeseries_dir = os.path.join(deriv_dir, "GLMdenoise", "%s" % model, "sub-wlsubj%03d" % subj)

for run in tqdm.tqdm(range(12)):
    print("Building timeseries dataframes...")

    fname = os.path.join(denoised_timeseries_dir, "denoised-timeseries_run-%02d.txt" % (run+1))
    timeseries = pd.read_csv(fname, sep = '\t', header = None)
    ...
    data = combined_df(prf_params, timeseries)
    ...
    tts_fname = os.path.join(deriv_dir, 'dataframes/%s/sub-wlsubj%03d/timeseries/denoiseddata_trialwise_timeseries_run-%02d.tsv' % (model, subj, run+1))
    tta_fname = os.path.join(deriv_dir, 'dataframes/%s/sub-wlsubj%03d/timeseries/denoiseddata_TTA_run-%02d.tsv' % (model, subj, run+1))
