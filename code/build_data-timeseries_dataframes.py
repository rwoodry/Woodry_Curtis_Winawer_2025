...
def slice_timeseries(timeseries_data, design, angles, window_size = 15, binsize = 10, mean_subtract=True,
    ...
    if mean_subtract:
        ...
        voxel_diff = run_timeseries - voxel_mean
        run_timeseries = voxel_diff / voxel_mean * 100
...
print("Building timeseries for: ", subj)


fname_pattern = os.path.join(deriv_dir, "fsnative_both/sub-wlsubj%03d/run_%02d.tsv")
...
for run in tqdm.tqdm(range(12)):
    Y_hat = pd.read_csv(fname_pattern % (subj, run), sep = '\t', header = None)
    ...
    if not os.path.exists(os.path.join(deriv_dir, 'dataframes/data/sub-wlsubj%03d/timeseries/' % subj)):
        os.makedirs(os.path.join(deriv_dir, 'dataframes/data/sub-wlsubj%03d/timeseries/' % subj))

    tts_fname = os.path.join(deriv_dir, 'dataframes/data/sub-wlsubj%03d/timeseries/data_trialwise_timeseries_run-%02d.tsv' % (subj, run+1))
    tta_fname = os.path.join(deriv_dir, 'dataframes/data/sub-wlsubj%03d/timeseries/data_TTA_run-%02d.tsv' % (subj, run+1))
